...
function get_arch() {
  ...
  case "${arch}" in
    ...
    i686) echo -n "386";;
    i386) echo -n "386";;
    *) msg_fatal "Sorry, ${arch} is not supported.";;
  esac
}
...
wget -qO- "https://github.com/koalaman/shellcheck/releases/download/${SHELLCHECK_VERSION}/shellcheck-${SHELLCHECK_VERSION}.$(tr '[:upper:]' '[:lower:]' < <(uname)).$(uname -m).tar.xz" | tar -xJf -
